...
-   Find and replace `Menuitem`/`Menuitems` with the table name (upper and lowercase B)
...
-   `curl -X POST -H "Content-Type: application/json" http://localhost:8000/menuitems -d @post-data.json |jq .`
-   `curl http://localhost:8000/menuitems |jq .`
